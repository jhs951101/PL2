...
ValueInfo PhoneContactList::find(string name) const {
	LCIter citer = findLocConst(name);
...
LCIter PhoneContactList::findLocConst (string name) const {
	LCIter citer;
	for (citer=contactTable.begin(); citer!=contactTable.end(); citer++){
...
string PhoneContactList::findName(int number) const {
	LCIter citer = findLocFromNumber(number);
...
string PhoneContactList::findName(string email) const {
	LCIter citer = findLocFromEmail(email);
...
LCIter PhoneContactList::findLocFromNumber(int number) const {
	LCIter citer;
...
LCIter PhoneContactList::findLocFromEmail(string email) const {
	LCIter citer;
